...
#define ACCESS_OBJECT_FIELD_BITFIELD(field_, type_)                            \
  _write_data_buffer_##type_(field_);
#define ACCESS_OBJECT_FIELD(field_, size_) _write_data_buffer(&(field_), size_);

void _scan_jsvalue_string(JSValue arm_value) {
  uint32_t tag = JS_VALUE_GET_NORM_TAG(arm_value);
  switch (tag) {
  case JS_TAG_STRING: {
    JSString *str = JS_VALUE_GET_STRING(arm_value);
    if (!str->atom_type) {
      FILL_PROTOCOL_DATA_ITEM(str,
                              sizeof(JSString) +
...
void _scan_object_jsvalue(JSObject *pobj) {
  if (pobj == NULL)
    return;
  JSObject *p = pobj;
  int i = 0;
  ...
  case JS_CLASS_ARGUMENTS: /* u.array | length */
    if (p->fast_array && p->u.array.u.values) {
      ...
      for (int i = 0; i < p->u.array.count; i++) {
        _scan_jsvalue_string(p->u.array.u.values[i]);
      }
    }
    break;
...
#endif
    _scan_jsvalue_string(p->u.object_data);
    break;
  ...
  case JS_CLASS_BOUND_FUNCTION: /* u.bound_function */
  {
    ...
    if (bf) {
      p_protocol_data_item ppdi = _alloc_protocol_data_item_header(
          sizeof(*bf) * 2 + sizeof(JSValue) * bf->argc);
      ppdi->arm_addr = (uint32_t)bf;
      ppdi->data_type = OBJECT_BOUND_FUNCTION;
      ppdi->buffer_size = _get_data_buffer_offset();
      JSBoundFunction_access(bf);
      if (bf->argc) {
        _write_data_buffer(bf->argv, sizeof(JSValue) * bf->argc);
      }
      ...
      _scan_jsvalue_string(bf->func_obj);
      _scan_jsvalue_string(bf->this_val);
      for (i = 0; i < bf->argc; i++) {
        _scan_jsvalue_string(bf->argv[i]);
      }
    }
  ...
  case JS_CLASS_C_FUNCTION_DATA: /* u.c_function_data_record */
  {
    ...
    if (fd) {
      for (i = 0; i < fd->data_len; i++) {
        _scan_jsvalue_string(fd->data[i]);
      }
    }
  } break;
...
void _scan_gc_object_list(JSRuntime *rt, struct list_head *gc_list) {
  struct list_head *el;
  list_for_each(el, gc_list) {
    ...
    case JS_GC_OBJ_TYPE_SHAPE: {
      ...
      ppdi->buffer_size = _get_data_buffer_offset();
      _write_data_buffer(sh_alloc, hash_size * sizeof(uint32_t));
      JSShape_access(sh);
      for (int idx = 0; idx < sh->prop_count; idx++) {
        JSShapeProperty_access(&(sh->prop[idx]));
      }
      ppdi->buffer_size = _get_data_buffer_offset() - ppdi->buffer_size;
    ...
    case JS_GC_OBJ_TYPE_JS_OBJECT: {
      ...
      {
        // 发送object
        FILL_PROTOCOL_DATA_ITEM_ALIGNMENT(obj, sizeof(JSObject),
      ...
      {
        // 发送prop
        FILL_PROTOCOL_DATA_ITEM(obj->prop,
          ...
          for (int i = 0; i < obj->shape->prop_count; i++) {
            ...
            if (prs->atom != JS_ATOM_NULL) {
              if (prs->flags & JS_PROP_TMASK) {
                ...
                _scan_jsvalue_string(pr->u.value);
              }
            }
          }
        }
      }
      _scan_object_jsvalue(obj);
    } break;
    case JS_GC_OBJ_TYPE_FUNCTION_BYTECODE: {
      ...
      p_protocol_data_item ppdi =
          _alloc_protocol_data_item_header(func_size * 2 + ext_size);
      ppdi->arm_addr = (uint32_t)func;
      ppdi->data_type = OBJECT_OBJECT_LIST;
      ppdi->buffer_size = _get_data_buffer_offset();
      JSFunctionBytecode_access(func);

      for (int i = 0; i < func->arg_count + func->var_count; i++) {
        JSVarDef_access(&func->vardefs[i])
      }
      for (int i = 0; i < func->closure_var_count; i++) {
        JSClosureVar_access(&func->closure_var[i])
      }
      // cpool
      for (int i = 0; i < func->cpool_count; i++) {
        JSCPool_access(func->cpool[i])
      }
      // byte_code
      _write_data_buffer(func->byte_code_buf, func->byte_code_len);
      ppdi->buffer_size = _get_data_buffer_offset() - ppdi->buffer_size;
      ppdi->shape_hash_size = 0;
      ppdi->shape_prop_size = 0;
      ppdi->object_type = gp->gc_obj_type;
      {
        // cpool
        for (int i = 0; i < func->cpool_count; i++) {
          _scan_jsvalue_string(func->cpool[i]);
        }
      }
      if (func->has_debug) {
        {
          FILL_PROTOCOL_DATA_ITEM(func->debug.pc2line_buf,
                                  func->debug.pc2line_len, OBJECT_DIRECT_COPY);
        }
        {
          FILL_PROTOCOL_DATA_ITEM(func->debug.source, func->debug.source_len,
                                  OBJECT_DIRECT_COPY);
        }
      }
      break;
    ...
    case JS_GC_OBJ_TYPE_VAR_REF: {
      _scan_jsvalue_string(((JSVarRef *)gp)->value);
      FILL_PROTOCOL_DATA_ITEM_ALIGNMENT((JSVarRef *)gp, sizeof(JSVarRef),
                                        OBJECT_OBJECT_LIST, JSVarRef_access);
      ppdi->object_type = gp->gc_obj_type;
      break;
    }
    case JS_GC_OBJ_TYPE_ASYNC_FUNCTION: {
      FILL_PROTOCOL_DATA_ITEM_ALIGNMENT(
          (JSAsyncFunctionData *)gp, sizeof(JSAsyncFunctionData),
    ...
    case JS_GC_OBJ_TYPE_JS_CONTEXT: {
      {
        FILL_PROTOCOL_DATA_ITEM_ALIGNMENT((JSContext *)gp, sizeof(JSContext),
                                          OBJECT_OBJECT_LIST, JSContext_access);
        ppdi->object_type = gp->gc_obj_type;
      }
      {
        JSContext *ctx = (JSContext *)gp;
        FILL_PROTOCOL_DATA_ITEM(ctx->class_proto,
      ...
      }
      {
        ...
        list_for_each(el, &((JSContext *)gp)->loaded_modules) {
          ...
          FILL_PROTOCOL_DATA_ITEM_ALIGNMENT(
              (JSModuleDef *)m, sizeof(JSModuleDef), OBJECT_JS_MODULE,
              JSModuleDef_access);
...
int _scan_rt_object(JSRuntime *rt) {
  int ret = 0;
  if (rt == NULL) {
    return ret;
  }

  JSRuntime *prt = (JSRuntime *)rt;

  _init_data_buffer(_g_data_buffer_size);
  for (int data_type_index = OBJECT_EMPTY; data_type_index < OBJECT_MAX;
       data_type_index++) {
    switch (data_type_index) {
    case OBJECT_RUNTIME: {
      FILL_PROTOCOL_DATA_ITEM_ALIGNMENT(prt, sizeof(JSRuntime), OBJECT_RUNTIME,
                                        JSRuntime_access);
    } break;
    case OBJECT_OBJECT_LIST: {
      _scan_gc_object_list(prt, &prt->gc_obj_list);
    } break;
    case OBJECT_SHAPE_HASH: {
      FILL_PROTOCOL_DATA_ITEM(prt->shape_hash,
                              sizeof(prt->shape_hash[0]) * prt->shape_hash_size,
                              data_type_index);
    } break;
    case OBJECT_ATOM_HASH: {
      FILL_PROTOCOL_DATA_ITEM(prt->atom_hash,
                              sizeof(prt->atom_hash[0]) * prt->atom_hash_size,
                              data_type_index);
    } break;
    case OBJECT_ATOM_ARRAY: {
      FILL_PROTOCOL_DATA_ITEM(prt->atom_array,
                              sizeof(prt->atom_array[0]) * prt->atom_size,
                              data_type_index);
    } break;
    case OBJECT_JS_STRING: {
      int str_len = 0;
      for (int i = 0; i < prt->atom_size; i++) {
        JSAtomStruct *p = prt->atom_array[i];
        if (!atom_is_free(p)) {
          if (p->len != 0) {
          ...
          FILL_PROTOCOL_DATA_ITEM(p, sizeof(JSString) + str_len,
                                  data_type_index);
        }
      }
    } break;
