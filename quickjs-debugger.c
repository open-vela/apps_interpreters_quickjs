...
static void js_send_validbp_event(JSDebuggerInfo* info, JSValue breakpoints) {
    JSContext *ctx = info->debugging_ctx;

    JSValue event = JS_NewObject(ctx);
    // better thread id?
    JS_SetPropertyStr(ctx, event, "type", JS_NewString(ctx, "ValidBreakpointEvent"));
    JSAtom atom = JS_NewAtom(ctx, "breakpoints");
    JS_DupValue(ctx, breakpoints);
    JS_SetProperty(ctx, event, atom, breakpoints);
    int64_t id = (int64_t)info->ctx;
    JS_SetPropertyStr(ctx, event, "thread", JS_NewInt64(ctx, id));
    js_transport_send_event(info, event);
...
static void js_process_breakpoints(JSDebuggerInfo *info, JSValue message) {
    ...
    //简单校验：如果文件存在，则向服务器确认断点有效
    if(access(path, F_OK) == 0) {
        js_send_validbp_event(info, message);
    }
...
static int js_process_debugger_messages(JSDebuggerInfo *info, const uint8_t *cur_pc) {
    ...
    do {
        ...
        printf("%s: [QJSDEBUGGER] receive: %s\n", __func__, info->message_buffer);
...
void js_debugger_check(JSContext* ctx, const uint8_t *cur_pc) {
    ...
    if (at_breakpoint) {
        ...
        printf("%s: hit breakpoint and call js_send_stopped_event(info, \"breakpoint\"); \n", __func__);
    }
...
int js_debugger_process_server_message(JSContext* ctx)
{
    JSDebuggerInfo *info = js_debugger_info(JS_GetRuntime(ctx));
    // 不做任何校验 直接跑逻辑
    // if (info->is_debugging)
    ...
    while (!info->is_paused) {
        ...
        if (!js_process_debugger_messages(info, NULL)) {
            info->ctx = ctx;
            return -1;
        }
    }
...
#ifdef CONFIG_INTERPRETERS_QUICKJS_DEBUG
void js_set_debugger_notify_callback(void* udata, void (*debugger_attach_notify)(JSContext* ctx, int type, void *udata))
{
    g_udata = udata;
    g_debugger_attach_notify = debugger_attach_notify;
}
#endif
...
void transport_close_inner(JSRuntime* rt, void *udata)
{
    JSDebuggerInfo *info = js_debugger_info(rt);
    if(info->debugger_attach_notify)
        info->debugger_attach_notify(info->ctx, 0, g_udata);
    info->transport_close_real(rt, udata);
}
...
void js_debugger_attach(
    ...
    void (*debugger_attach_notify)(JSContext* ctx, int type, void *udata),
    void *udata
) {
    ...
    info->transport_close_real = transport_close;
    //套一层外层函数，内部调用真正的关闭函数，同时会触发debugger_attach_notify
    info->transport_close = transport_close_inner;
    info->debugger_attach_notify = debugger_attach_notify;
    info->transport_udata = udata;

    // 初始化消息处理
    if(debugger_attach_notify)
        debugger_attach_notify(ctx, 1, g_udata);
